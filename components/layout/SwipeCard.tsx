...
export const SwipeCard: React.FC<SwipeCardProps> = ({
    ...
    (action: SwipeAction) => {
      ...
      gsap.to(cardElement, {
        ...
        onComplete: () => {
          ...
          gsap.set(cardElement, { x: 0, y: 0, rotation: 0, opacity: 1, clearProps: "all" });
        },
      });
    },
  ...
  useLayoutEffect(() => {
    ...
    draggableInstance.current = Draggable.create(cardElement, {
      ...
      onDrag: function () {
        ...
        if (x > 0) {
          gsap.to(longBadge, { opacity: intensity, scale: 1 + (intensity * 0.1), duration: 0.1 });
          gsap.to(shortBadge, { opacity: 0, duration: 0.1 });
          gsap.to(overlay, { backgroundColor: `rgba(16, 185, 129, ${intensity * 0.4})`, opacity: 1, duration: 0.1 });
        } else {
          gsap.to(shortBadge, { opacity: intensity, scale: 1 + (intensity * 0.1), duration: 0.1 });
          gsap.to(longBadge, { opacity: 0, duration: 0.1 });
          gsap.to(overlay, { backgroundColor: `rgba(244, 63, 94, ${intensity * 0.4})`, opacity: 1, duration: 0.1 });
        }
      },
      onRelease: function () {
        const x = this.x;
        ...
        if (x > threshold) handleSwipeComplete("long");
          ...
          gsap.to(cardElement, { x: 0, y: 0, rotation: 0, duration: 0.5, ease: "elastic.out(1, 0.75)" });
          gsap.to([longBadge, shortBadge, overlay], { opacity: 0, duration: 0.3 });
        }
      },
    });
  ...
  if (!hasCard) {
    return (
      <div className="flex flex-col items-center justify-center h-[calc(100vh-140px)] text-neutral-500 animate-in fade-in">
        ...
        <button 
          onClick={() => { setFilterCountry(null); setShuffledNews(shuffleArray(newsList)); setActiveIndex(0); }}
  ...
  return (
    // MAIN CONTAINER: Fixed Height, No Scroll, Perfect Center
    <div className="flex flex-col items-center justify-between w-full max-w-md mx-auto h-[calc(100vh-100px)] py-2 overflow-hidden">
        ...
        <div className="flex gap-2 justify-center">
          ...
          {countries.map((c) => {
             ...
             return (
               ...
               >
                 {cCode && <ReactCountryFlag countryCode={cCode} svg className="rounded-full w-3 h-3" />}
                 {c}
               </button>
             )
          })}
      ...
      <div className="relative flex-1 w-full flex justify-center items-center perspective-1000 min-h-0 my-4">
        ...
        >
          <div data-overlay className="absolute inset-0 pointer-events-none z-20 mix-blend-overlay transition-opacity" />
          ...
          <div className="relative h-[55%] w-full">
            ...
            <div className="absolute top-4 left-4 z-40 bg-black/40 backdrop-blur-md border border-white/10 rounded-full px-3 py-1 flex items-center gap-2">
               {code && <ReactCountryFlag countryCode={code} svg className="w-4 h-4 rounded-full" />}
               <span className="text-[10px] font-bold text-white tracking-wide uppercase">{currentNews.country}</span>
            </div>
          </div>
